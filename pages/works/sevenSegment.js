...
const Work = () => {
  ...
  const getDisplayValueBinary = valor => {
    ...
    if (valor == 0) {
      const binaryValue = segmentOrder
        .map(segment => (segmentStates[segment] ? '1' : '0'))
        .join('')
      return binaryValue
    }
    if (valor == 1) {
      const binaryValue = segmentOrder
        .map(segment => (segmentStates[segment] ? '0' : '1'))
        .join('')
      return binaryValue
    }
  }
  ...
  return (
    <Layout title="SevenSegmemt">
      <Container>
        ...
        <P>
          Existen variaciones en el diseño de los displays de 7 segmentos, como
          los displays de ánodo común y cátodo común. En los displays de ánodo
          común, los segmentos se activan conectando los cátodos de los LEDs a
          tierra, mientras que en los displays de cátodo común, los segmentos se
          activan conectando los ánodos de los LEDs a un voltaje positivo.
        </P>
